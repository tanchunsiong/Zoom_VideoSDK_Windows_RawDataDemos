...
void MainFrame::onSessionJoin()
{
	...
	if (video_sdk_obj_)
	{
		...
		if (videohelper) {
			...
			setting.mode = ZoomVideoSDKVideoPreferenceMode_Balance;
			...
			//if there is USB capture device, send USB capture device as main camera
			//and send webcam as multicam
			if (usbCaptureDevice) {
				videohelper->selectCamera(usbCaptureDevice->getDeviceId());
				ZoomVideoSDKErrors err = videohelper->startVideo();

				//send webcam as multicam, this is sending camera which is in hardcoded index 0

				//sharehelper->enableOptimizeForSharedVideo(true);
				int index = 0;
				IZoomVideoSDKCameraDevice* pCamera = pCameraList->GetItem(index);
				boolean multistreamSuccess = videohelper->enableMultiStreamVideo(pCamera->getDeviceId(), pCamera->getDeviceName());
				if (multistreamSuccess) { //return ZoomVideoSDKErrors_Success;
				}
				else {// return ZoomVideoSDKErrors_Unknown; 
				}
			
			}

			//if there is no USB Capture device, send webcamp as main cam. This is hardcoded index of 0
			else {
				int index = 0;
				IZoomVideoSDKCameraDevice* cam = videohelper->getCameraList()->GetItem(index);
				videohelper->selectCamera(cam->getDeviceId());
